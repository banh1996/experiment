...
class Hw01 {
    ...
    public Node delete_node(Node root_node, int val) {
        ...
        if (root_node == null)
            return null;
        if (root_node.val > val) { //search in left side
            root_node.left = delete_node(root_node.left, val);
        } else if (root_node.val < val) { //search in right side
            root_node.right = delete_node(root_node.right, val);
        } else { //found it, delete
            is_deleted = 1;
