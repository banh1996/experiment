...
static void* map4_thread(void* p)
{
...
static void* map4(void* p)
{
    word_index_t *word_indexes[13];
    pthread_t tid[13];
    pthread_attr_t tattr;
    struct sched_param param;
    int newprio = THREAD_PRIORITY_NORMAL;
    //int ret = 0;
    (void)p;
    ...
    for (int i = 0; i < 13; i++)
    {
        param.sched_priority = newprio + 1;
        pthread_attr_setschedparam (&tattr, &param);
        pthread_create(&tid[i], &tattr, map4_thread, (void*)word_indexes[i]);
    }
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&id1, NULL, map4, NULL);
    pthread_create(&id2, NULL, reduce4, NULL);
