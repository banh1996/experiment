...
public class Evaluator implements Visitor<Value> {
	...
	public static boolean check_value(int op, Value v1, Value v2) {
		if (v1 instanceof NumVal && v2 instanceof NumVal) {
			NumVal first = (NumVal) v1;
			NumVal second = (NumVal) v2;
			if (first.v() == second.v() && op == 0)
				return true;
			else if (first.v() > second.v() && op == 1)
				return true;
			else if (first.v() < second.v() && op == -1)
				return true;
			return false;
			...
			if (s1.equals(s2) && op == 0)
				return true;
			else if (s1.length() > s2.length() && op == 1)
				return true;
			else if (s1.length() < s2.length()  && op == -1)
				return true;
			return false;
		} else if (v1 instanceof PairVal && v2 instanceof PairVal) {
			boolean b1 = check_value(op, ((PairVal)v1).fst(), ((PairVal)v2).fst());
			boolean b2 = check_value(op, ((PairVal)v1).snd(), ((PairVal)v2).snd());
			return b1 && b2 && (op == 0);
		} else if (v1 instanceof FunVal && v2 instanceof FunVal) {
			return (v1 == v2) && (op == 0);
		} else if (v1 instanceof BoolVal && v2 instanceof BoolVal) {
			return (((BoolVal)v1).v() == ((BoolVal)v2).v()) && (op == 0);
	...
	public Value visit(LessExp e, Env env) { // New for funclang.
		...
		return new Value.BoolVal(check_value(-1, first, second));
	}
	...
	public Value visit (EqualExp e, Env env) { // New for funclang.
		...
		return new BoolVal(check_value(0, v1, v2)) ;
	}
